...
import { loginMember } from "@/app/actions/member-actions";
        ...
        <div className="bg-foreground/[0.02] border border-foreground/10 rounded-[3rem] p-8 md:p-16 backdrop-blur-xl shadow-2xl relative overflow-hidden">
          ...
          <div className="relative z-10">
            <div className="text-center mb-12 space-y-4">
              ...
              <p className="opacity-60 text-sm">Welcome back. Enter your credentials to access your vault.</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full bg-foreground text-background py-5 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-v6-accent hover:text-white transition-all flex justify-center items-center gap-2 mt-4 disabled:opacity-50"
              >
            ...
            <div className="mt-8 text-center">
              <p className="text-xs opacity-40 font-medium">
                ...
                <Link href="/join" className="text-v6-accent hover:text-foreground font-black transition-colors">Sign up</Link>
              </p>
            </div>
          </div>
        </div>
...
export default function LoginPage() {
  return (
    <main className="min-h-screen bg-background text-foreground flex flex-col">
      <Header />
      <Suspense fallback={<div className="flex-1 flex items-center justify-center"><Loader2 className="animate-spin opacity-30" /></div>}>
        <LoginForm />
      </Suspense>
      <Footer />
    </main>
